...
class Boss2 extends Monster{
  constructor(x,y){
    ...
    this.bodies.push(new boss2_1_body1(this.x,this.y,0,1000));
  }
  shoot(hero){
    if(this.skill==0){
      if(this.shootCount>this.shootRate){
        ...
        if(this.skillCount<10){
          this.bullets.push(new boss2_bullet(this.v.x,this.v.y,hero,this.size,this.little_bullet,0));
          this.skillCount++;  
        }
        else{
      ...
      if(this.shootCount>this.shootRate){
        this.shootCount=0;
        this.bullets.push(new boss2_bullet(this.v.x,this.v.y,hero,this.size,this.little_bullet,0));
        this.bullets.push(new boss2_bullet(this.v.x,this.v.y,hero,this.size,this.little_bullet,45));
        this.bullets.push(new boss2_bullet(this.v.x,this.v.y,hero,this.size,this.little_bullet,90));
        this.bullets.push(new boss2_bullet(this.v.x,this.v.y,hero,this.size,this.little_bullet,135));
        this.bullets.push(new boss2_bullet(this.v.x,this.v.y,hero,this.size,this.little_bullet,180));
        this.bullets.push(new boss2_bullet(this.v.x,this.v.y,hero,this.size,this.little_bullet,225));
        this.bullets.push(new boss2_bullet(this.v.x,this.v.y,hero,this.size,this.little_bullet,270));
        this.bullets.push(new boss2_bullet(this.v.x,this.v.y,hero,this.size,this.little_bullet,315));
        this.skill_n--;
      }
      if(this.skill_n<=0){
        this.skill=0;
      ...
      if(this.shootCount>this.shootRate){
        this.shootCount=0;
        this.bullets.push(new boss2_bullet(this.v.x,this.v.y,hero,this.size,this.little_bullet,0));
        this.bullets.push(new boss2_bullet(this.v.x,this.v.y,hero,this.size,this.little_bullet,30));
        this.bullets.push(new boss2_bullet(this.v.x,this.v.y,hero,this.size,this.little_bullet,-30));
        this.skill_n--;
      }
      if(this.skill_n<=0){
  ...
  move(){
    ...
    let goal_v;
    if(this.v.y<height*0.5-this.height){
...
class boss2_1_body1{
  ...
  display(skill){
    if(skill==0){
      image(img_boss2_1,this.x,this.y,img_boss2_1.width*ratio,img_boss2_1.height*0.5*ratio);
    }
    else if(skill==1){
    ...
    if(this.life>0){
      fill('red');
      textSize(20*ratio);
      text(this.life+1000,this.x,this.y);
    }
  }
  move(diffV){
    this.x+=diffV.x;
